    ...
    >
      ...
      <div style={{ aspectRatio: '9/16', width: '100%', position: 'relative', marginBottom: '10px', borderRadius: '2px', background: '#0A0908', border: '1px solid #1F1D1A', overflow: 'hidden' }}>
        ...
        {!(project.status === 'Completed' && videoUrl) && !hasThumb && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '6px', opacity: 0.25 }}>
            ...
            <span style={{ fontSize: '0.75rem', color: '#6B6560' }}>No preview</span>
          </div>
        )}
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '8px' }}>
        <div>
          ...
          <p style={{ fontSize: '0.78rem', color: '#6B6560' }}>{date}</p>
        </div>
        <StatusBadge status={project.status} />
      </div>

      {project.revision_count > 0 && (
        <p style={{ fontSize: '0.78rem', color: '#6B6560', marginTop: '6px' }}>
...
const EmptyState = ({ onCreate }) => (
  <div style={{ gridColumn: '1 / -1', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '96px 32px', gap: '24px' }}>
    ...
    <div style={{ textAlign: 'center' }}>
      ...
      <p style={{ fontSize: '0.88rem', color: '#6B6560', fontWeight: 300 }}>Your first creation is one click away.</p>
    </div>
      ...
      <div style={{ minHeight: '100vh', background: '#0A0908', color: '#F4F1EA', fontFamily: "'General Sans','Inter',-apple-system,sans-serif", fontWeight: 300 }}>
        ...
        <header style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            ...
            <span style={{ fontSize: '0.82rem', color: '#6B6560' }} className="hidden sm:block">{profile?.email}</span>
            <button
              onClick={async () => { await signOut(); router.replace('/') }}
              style={{
                background: 'none', border: '1px solid #1F1D1A', color: '#6B6560',
              ...
              onMouseLeave={e => { e.currentTarget.style.color = '#6B6560'; e.currentTarget.style.borderColor = '#1F1D1A' }}
            >
              Sign out
            </button>
          </div>
        ...
        <main style={{ maxWidth: '1200px', margin: '0 auto', padding: '64px 40px' }}>
          ...
          <div style={{ marginBottom: '48px' }}>
            ...
            <p style={{ fontSize: '0.88rem', color: '#6B6560', fontWeight: 300 }}>
              {projects.length > 0
        ...
        <footer style={{ borderTop: '1px solid #1F1D1A', padding: '24px 40px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
          ...
          <div style={{ display: 'flex', gap: '20px' }}>
            <Link href="/terms" style={{ fontSize: '0.78rem', color: '#6B6560', textDecoration: 'none' }}>Terms</Link>
            <Link href="/privacy" style={{ fontSize: '0.78rem', color: '#6B6560', textDecoration: 'none' }}>Privacy</Link>
            <a href="mailto:hello@yourvision.video" style={{ fontSize: '0.78rem', color: '#6B6560', textDecoration: 'none' }}>Contact</a>
          </div>
        </footer>
